...
- One or more file paths → read each, propose `file edit capability`s, confirm before writing.
- Ambiguous ("humanise my post") → use `the question tool` to pick a mode. If the input is over 500 words or the user mentioned a sample, ask one more question about voice calibration: match a pasted sample, match a file, or default natural voice.

### 2. Load patterns on demand

The self-check literals below cover short inputs. file read capability `PATTERNS.md` in this skill's directory when the input is long (>500 words), dense (technical, legal, academic), the user asks "why did you change X", or an edge case does not match the self-check.
...
1. Propose `file edit capability`s file by file, wait for acceptance.
