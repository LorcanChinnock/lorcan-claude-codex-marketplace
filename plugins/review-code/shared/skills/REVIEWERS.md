...
Verbatim briefs for the six specialist reviewers spawned in SKILL.md step 4. Launch selected reviewers in parallel — one `delegation/subagent capability` call per reviewer, all in a single message.
...
4. Absolute paths of relevant project instruction files.
...
Use strong reasoning worker for every specialist unless noted. Use the briefs below verbatim.
...
> file read capability each listed project instruction files file, then audit the diff for violations. project instruction files guidance applies to runtime-authored code; not every rule is a reviewable standard. Flag only rules directly violated by the diff. Also read the non-test source files touched by the diff and check whether changes contradict inline comments or explicit conventions visible in surrounding code. Cite the project instruction files rule or the comment verbatim for each finding.
