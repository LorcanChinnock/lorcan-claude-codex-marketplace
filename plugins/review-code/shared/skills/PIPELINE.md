...
git diff <merge-base>...HEAD --name-only     # for project instruction files location
```
...
## §2 — Three lightweight worker pre-steps in parallel

Spawn three `delegation/subagent capability` calls in a single message. All use lightweight worker for cost.
...
- Root `project instruction files` of the repo.
- Any `project instruction files` in ancestor directories of each changed file.
