...
Each spoke receives: scope files, scope regions, contract (from step 4), ancestor `project instruction files` paths, stack info. Each returns a list of proposals as `{file, lines, current, proposed, rationale, spoke}`. Spokes are read-only — no edits, no writes.

## Spoke briefs (pass verbatim)

### Dead-weight (strong reasoning worker)
...
### Redundancy (strong reasoning worker)
...
### Modernization (strong reasoning worker, uses context7 MCP)
...
### Control-flow (strong reasoning worker)
...
### Boundary-audit (strong reasoning worker)
...
## Validator brief (step 6, pass verbatim — one strong reasoning worker `delegation/subagent capability` per proposal, in parallel)
...
## Scoring rubric (step 7, pass verbatim — one lightweight worker `delegation/subagent capability` per proposal, in parallel)
...
Print this exactly before any `file edit capability`:
...
- Style nits not backed by a `project instruction files` rule or a verified current idiom.
...
- Every `delegation/subagent capability` call must pass `model` explicitly. lightweight worker for steps 2, 3, 7; strong reasoning worker for steps 4, 5, 6.
- Parallel steps (5, 6, 7) must be one message with multiple `delegation/subagent capability` uses. Sequential dispatch when parallel is possible is a bug.
