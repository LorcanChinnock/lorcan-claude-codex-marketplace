...
- [`coralogix`](plugins/coralogix) — query Coralogix logs, traces, and metrics, design alerts, and design dashboards via the Coralogix MCP. Three skills: query (structured report), design-alert (interactive JSON drafting with explicit confirmation before writes), design-dashboard (Markdown spec the user builds in the UI). Read-only by default; does not bundle its own MCP server.
