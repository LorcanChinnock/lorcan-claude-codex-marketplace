...
### 1. Read the shared style and the template
...
- [TEMPLATE.md](TEMPLATE.md) — section structure for feature-design docs
...
Ask closed questions first as a single `AskUserQuestion` call. One call, four questions, no multi-select:
...
Write the draft using [TEMPLATE.md](TEMPLATE.md) as the section skeleton. Fill from the questionnaire answers plus whatever the conversation already established.
...
Write the draft to the chosen path using `Write`.
