...
For every issue returned in step 4, spawn a lightweight worker `delegation/subagent capability` in parallel with: the full issue, the diff, the project instruction files paths. Prompt verbatim:
...
> - **25**: Might be real, unverified. If stylistic, not explicitly in a relevant project instruction files.
> - **50**: Verified real, but a nit or rare in practice. Low importance relative to the change.
> - **75**: Verified, likely to bite in practice, or directly named in a relevant project instruction files. Current approach insufficient.
> - **100**: Directly confirmed by evidence, will happen frequently.
>
> If the issue cites a project instruction files rule, open the file and confirm the rule is actually there and applies. If not, score ≤25.
>
...
Print the report to the conversation. Never call `gh pr comment`, `gh pr review`, or any mutating `gh` / `git` command. If the caller explicitly asked for a file, use `file write capability`; otherwise print only.
...
1. [confidence: <n>] <brief description>. <Reason: project instruction files / bug / security / perf / history / comment-verified>.
...
- Missing tests / docs / general "code quality" unless project instruction files names the rule.
- Style nits not named in a relevant project instruction files.
