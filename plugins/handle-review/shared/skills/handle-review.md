...
### 1. file read capability the full feedback

file read capability every item before acting. Do not react mid-read.
...
- Verify against the codebase: open the file, find callers, check the test, run the build. Prefer the `language-aware navigation` tool (definitions, references, hover, symbols) over `grep` when checking call sites or types — it's language-aware. Fall back to `grep`/`file read capability` when no language-aware navigation is loaded.
...
Apply [PATTERNS.md](../../skills/handle-review/PATTERNS.md) for push-back templates.

### 4. Acknowledge correct feedback in the code

When feedback is right, fix it and say what changed in one line. Before replying, scan the draft against the forbidden-phrase list in [PATTERNS.md](../../skills/handle-review/PATTERNS.md) and rewrite any match. If you catch yourself typing "Thanks", delete it. Acknowledgement templates also live in [PATTERNS.md](../../skills/handle-review/PATTERNS.md).
...
read-only `gh` commands for pulling PR metadata, inline comments, and the diff live in [GH.md](../../skills/handle-review/GH.md). Never use `gh` to post, merge, approve, or request changes. Writes belong to the user.
...
- No forbidden phrases (see [PATTERNS.md](../../skills/handle-review/PATTERNS.md)).
